defmodule Agentleguide.Jobs.HistoricalEmailSyncJob do
  ...
  def perform(%Oban.Job{args: args}) do
    ...
    case Accounts.get_user(user_id) do
      ...
      user ->
        if oldest_date do
          Logger.info("HistoricalEmailSyncJob: Resuming historical email sync for user #{user.email} from #{oldest_date} (#{total_synced} emails synced so far)")
        else
          Logger.info("HistoricalEmailSyncJob: Starting historical email sync for user #{user.email}")
        end

        case sync_emails_with_resume(user, oldest_date, total_synced, page_token) do
          {:ok, final_total} ->
            Logger.info("HistoricalEmailSyncJob: Successfully synced #{final_total} historical emails for user #{user.email}")
          ...
          {:continue, new_args} ->
            ...
            Logger.info("HistoricalEmailSyncJob: Scheduling continuation for user #{user.email} - #{new_args["total_synced"]} emails synced so far")
            %{user_id: user_id}
            |> Map.merge(new_args)
            |> __MODULE__.new()
            |> Oban.insert()
          ...
          {:error, reason} ->
            Logger.error("HistoricalEmailSyncJob: Failed to sync emails for user #{user.email}: #{inspect(reason)}")
            {:error, reason}
        end
    end
  end
  ...
  defp sync_emails_with_resume(user, oldest_date, total_synced, page_token) do
    ...
    query_params = if oldest_date do
      %{"maxResults" => "100", "pageToken" => page_token}
    else
      %{"maxResults" => "100"}
    end

    query_params = if page_token, do: Map.put(query_params, "pageToken", page_token), else: query_params
  ...
  defp sync_emails_batch_with_resume(user, oldest_date, total_synced, query_params, batch_count) do
    Logger.info("HistoricalEmailSyncJob: Fetching batch #{batch_count + 1} for user #{user.email} (#{total_synced} synced so far)")
    ...
    case fetch_email_batch(user, query_params["pageToken"]) do
      {:ok, %{message_ids: message_ids, next_page_token: next_token}} ->
        ...
        Logger.info("HistoricalEmailSyncJob: #{length(new_message_ids)} new emails to fetch and store")

        # Fetch and store the new emails
        Logger.info("HistoricalEmailSyncJob: Starting to fetch and store #{length(new_message_ids)} emails...")
        {:ok, results} = GmailService.fetch_and_store_messages(user, new_message_ids)
        ...
        new_oldest_date = if length(results) > 0 do
          results
        ...
        Logger.info("HistoricalEmailSyncJob: Synced batch of #{batch_synced} emails (total: #{new_total}) for user #{user.email}")
        ...
        if next_token && (batch_count >= 9 || rem(new_total, 1000) == 0) do
          ...
          if next_token do
            ...
            sync_emails_batch_with_resume(user, new_oldest_date, new_total, new_query_params, batch_count + 1)
          else
            {:ok, new_total}
          end
        end
    ...
    case Finch.build(:get, url, headers)
      ...
      {:ok, %{status: 200, body: body}} ->
        case Jason.decode(body) do
          ...
          {:error, error} ->
            Logger.error("HistoricalEmailSyncJob: Failed to parse Gmail response: #{inspect(error)}")
            {:error, error}
        end

      {:ok, %{status: 401}} ->
        Logger.error("HistoricalEmailSyncJob: Gmail API authentication failed for user #{user.id}")
        {:error, :auth_failed}
